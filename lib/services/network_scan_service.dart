...
class NetworkScanService {
  ...
  Future<String?> getDeviceIp() async {
    try {
      ...
      for (final interface in interfaces) {
        for (final addr in interface.addresses) {
          if (!addr.address.startsWith('169.254') && !addr.address.startsWith('192.0.0')) {
            return addr.address;
          }
        }
      }
  ...
  static const List<int> commonPorts = [21, 22, 23, 80, 443, 3389, 445, 8000, 8080];
  ...
  Future<Map<String, List<int>>> scanNetwork(String subnetPrefix, {
    int maxHost = 254,
    void Function(int checked, int total)? onProgress,
  }) async {
    final results = <String, List<int>>{};
    const hostBatchSize = 10; // scan up to 10 addresses at once, not one at a time
    ...
    for (int start = 1; start <= maxHost; start += hostBatchSize) {
      ...
      for (int i = start; i <= end; i++) {
        ...
        batchFutures.add(_scanPortsForHost(ip).then((portResults) {
          final openPorts = portResults.entries.where((e) => e.value).map((e) => e.key).toList();
