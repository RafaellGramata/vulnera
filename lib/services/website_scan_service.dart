...
class WebsiteScanService {
  Future<List<WebsiteFinding>> scanUrl(String url) async {
    ...
    try {
      ...
      throw WebsiteScanException('Could not reach the site. Check the URL and try again.');
    }
    ...
    if (!headers.containsKey('strict-transport-security')) {
      findings.add(WebsiteFinding(
        ...
        description: 'The Strict-Transport-Security header is missing, so browsers will not automatically enforce HTTPS on future visits.',
        severity: 'Medium',
        cvssScore: 5.9,
      ));
    }
    ...
    if (!headers.containsKey('content-security-policy')) {
      findings.add(WebsiteFinding(
        ...
        description: 'No Content-Security-Policy header was found, leaving the site more exposed to cross-site scripting (XSS) attacks.',
        severity: 'Medium',
        cvssScore: 6.1,
      ));
    }
    ...
    if (!headers.containsKey('x-frame-options')) {
      findings.add(WebsiteFinding(
        ...
        description: 'No X-Frame-Options header was found, making the site potentially vulnerable to clickjacking attacks.',
        severity: 'Low',
        cvssScore: 3.7,
      ));
    }
    ...
    if (!headers.containsKey('x-content-type-options')) {
      findings.add(WebsiteFinding(
        ...
        description: 'No X-Content-Type-Options header was found, which can allow browsers to misinterpret file types (MIME sniffing).',
        severity: 'Low',
        cvssScore: 3.1,
      ));
    }
